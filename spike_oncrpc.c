 ...
 * none
 */
void s_onc_call_header(uint xid, uint prognum, uint progver, uint procnum)
{
 ...
 * none
 */
void s_onc_opaqueauth_sys(uint stamp, /* time stamp usually */
...
int s_onc_push_random_bytes(uint size)
{
	uint i = 0;
 ...
 */
int s_binary_block_size_onc_recordfrag(char* blockname)
{
	return s_binary_block_size_word_bigendian_plussome(blockname, 0x80000000);
}
 ...
 */
int s_onc_do_portmap_getport(char* host, struct onc_endpoint* endpoint)
{
	...
	responsedatasize = s_onc_parse_response(responsebuf, responsebufsize, responsedata, responsedatasize);
 ...
 */
int s_onc_do_portmap_dump(char* host, struct onc_endpoint* eparray, uint arraysize)
{
	struct spike* old_spike;
	struct spike* onc_spike;
	unsigned char responsebuf[BIG_BUF], responsedata[BIG_BUF];
	...
	for (i = 0; i < arraysize; i++)
	{
		...
		if (processed + 4 <= responsedatasize)
		{
			eparray[i].prognum = ntohl(*(uint *)(responsedata + processed));
			processed += 4;
		}
		else
		{
		...
		if (processed + 4 <= responsedatasize)
		{
			eparray[i].progver = ntohl(*(uint *)(responsedata + processed));
			processed += 4;
		}
		else
		{
		...
		if (processed + 4 <= responsedatasize)
		{
			eparray[i].proto = ntohl(*(uint *)(responsedata + processed));
			processed += 4;
		}
		else
		{
		...
		if (processed + 4 <= responsedatasize)
		{
			eparray[i].port = ntohl(*(uint *)(responsedata + processed));
			processed += 4;
		}
		else
		{
 ...
 * 1 on success, 0 on failure
 */
int s_onc_read_reply(struct spike* cur_spike, unsigned char* mesgbuf, uint* mesgbufsize)
{
	...
	{
		ret = read(cur_spike->fd, buffer + total + size, BIG_BUF - (total + size));
		if (ret >= 0)
		{
			size += ret;
		}
		...
		{
			fragsize = ntohl(*(uint *)(buffer + total)) & 0x7FFFFFFF;
 ...
 */
int s_onc_parse_response(unsigned char* responsebuf, uint responsebufsize, unsigned char* responsedata, uint responsedatasize)
{
	...
	while (remaining)
	{
		...
		if (remaining > 4)
		{
			fragsize = ntohl(*(uint *)(buffer + processed)) & 0x7FFFFFFF;
		...
		if (xid)
		{
			tmpuint = fragsize < (responsedatasize - rdprocessed) ? fragsize : (responsedatasize - rdprocessed);
			memcpy(responsedata + rdprocessed, buffer + processed, tmpuint);
			rdprocessed += tmpuint;
			processed += fragsize;
		...
		case 1:
			...
			case 0:
				...
				tmpuint = tmpuint < (responsedatasize - rdprocessed) ? tmpuint : (responsedatasize - rdprocessed);
				memcpy(responsedata + rdprocessed, buffer + processed, tmpuint); 
				rdprocessed += tmpuint;
				processed += (fragsize + 4) - (processed - tmp);
				remaining = responsebufsize - processed;
				break;
			case 1:
