...
int dldemarshall(void** arg, int dataType, unsigned char* data)
{
...
int parse_quoted_string(char* in, char** newarg, char** next)
{
	char* c, * d;
...
int parse_int(unsigned char* in, unsigned char** newarg, unsigned char** next)
{
	char* c, * d;
	char* newposition;
	int length;
...
abcd2",123);
*/
int get_phrase(unsigned char* start, unsigned char** newstring, int state, unsigned char** next)
{
	char* c;
	int result;
...
int s_parse_line(char* line, char* currentvariable, int linenumber, struct dlargs* mydlargs, int state, char* c)
{
