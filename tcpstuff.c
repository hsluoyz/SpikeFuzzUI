...
int make_tcp_listener(unsigned short localport, int* fd)
{
	int sfd = -1;
...
int tcp_accept(int listenFd)
{
	...
	lingerVal.l_onoff = 0;
	lingerVal.l_linger = 0;
	if (setsockopt(clientFd, SOL_SOCKET, SO_LINGER, (char*)&lingerVal, sizeof(lingerVal)) < 0)
	{
...
int tcpconnect(const char* host, const unsigned short port, int getreserved)
{
	int sfd = -1, p = -1; 
	struct sockaddr_in addr;
	struct linger lingerVal;
...
int tcpread(int fd, uint32 size, unsigned char* buffer)
{
	uint32 left;
	unsigned char* p;
	int i;
...
int tcpwrite(int fd, uint32 size, unsigned char* inbuffer)
{
	uint32 left;
	unsigned char* p;
	int i;
