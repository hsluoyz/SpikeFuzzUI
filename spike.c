...
void s_update_listeners(unsigned long size)
{
	int i;

	for (i = 0; i < MAXLISTENERS; i++)
	{
		if (current_spike->block_listeners[i].isused && current_spike->block_listeners[i].isopen)
		{
...
int s_binary(char* instring)
{
   ...
   printf("S_binary() at %d\n",i);
		...
		/*
							...
							*/
		if ((instring[i] == '0' && instring[i + 1] == 'x') || (instring[i] == '\\' && instring[i + 1] == 'x'))
		{
			i += 2;
			continue;
		}
...
int add_size_listener(int size, char* instring, int type, long plussome, float mult)
{
	listener* myListener;
...
int is_int_fuzz_variable(int type)
{
	/*is it our turn?*/
...
int s_blocksize_signed_string_variable(char* instring, int size)
{
	int retval = 1;
...
int s_binary_block_size_halfword_bigendian_variable(char* blockname)
{
	int retval = 1;
...
int s_binary_block_size_intel_halfword_plus_variable(char* blockname, long plus)
{
	int retval = 1;
	if (!is_int_fuzz_variable(BINARYLITTLEENDIANHALFWORD))
	{
...
int s_binary_block_size_halfword_bigendian_mult(char* blockname, float mult)
{
...
int s_binary_block_size_word_intel_mult_plus(char* blockname, long some, float mult)
{
#if 1
	printf("mult is %f\n", mult);
#endif
...
void close_a_size(listener* myListener, unsigned long size)
{
	long addme;
#ifdef DEBUG
...
void close_sizes(char* blockname, unsigned long size)
{
	int i;
...
unsigned long unintel(unsigned long myint)
{
	unsigned long tint;
	/*first, we reverse it*/
	/*
...
void s_int_variable(int defaultvalue, int type)
{
...
void s_unistring_variable(unsigned char* variable)
{
...
void s_add_fuzzint(unsigned long fuzzint)
{
	int i;
	i = s_maxfuzzints++;
...
void s_add_fuzzstring(unsigned char* newfuzzstring)
{
	int i;
...
void s_add_long_fuzzchars(unsigned char fuzzchar)
{
...
unsigned char* s_scan_for_variable(unsigned char* buffer, unsigned char* startdelim, unsigned char* enddelim)
{
