...
int send_ntlm_packet(unsigned char* packet, char* method)
{
...
/* does both calls*/
int get_ntlm_page(char* url, char* user, char* password, char* domain, char* responsebuf)
{
	...
	while (retval && notfin)
	{
		...
		if (retval)
		{
			...
			if (response == 0)
			{
				if ((response = s_scan_for_variable(buf, "WWW-Authenticate: NTLM ", "\r\n")) != NULL)
				{
					/*found our string */
	...
	buildSmbNtlmAuthResponse((tSmbNtlmAuthChallenge *)buf, (tSmbNtlmAuthResponse *)buf2, user, password);
