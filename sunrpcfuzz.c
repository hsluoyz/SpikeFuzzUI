 ...
 */
int fuzzprog(char* host, struct onc_endpoint ep, uint iterations, uint minprocnum, uint maxprocnum, uint number_of_items)
{
	uint i = 0;
	uint j = 0;
		...
		for (j = 0; j < iterations; j++)
		{
			...
			else
			{
				/* we create the AUTH_SYS authenticator with
						...
						* malicious purposes.
						*/
				s_onc_opaqueauth_sys((uint)rand(), j == 2 ? bogushost : ourhost, getuid(), getgid(), &gids, 1);
			}
